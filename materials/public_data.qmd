...
for i in {1..22}
do
  wget -O chr${i}.vcf.gz "http://ftp.1000genomes.ebi.ac.uk/vol1/ftp/data_collections/1000_genomes_project/release/20181203_biallelic_SNV/ALL.chr${i}.shapeit2_integrated_v1a.GRCh38.20181129.phased.vcf.gz"
  wget -O chr${i}.vcf.gz.tbi "http://ftp.1000genomes.ebi.ac.uk/vol1/ftp/data_collections/1000_genomes_project/release/20181203_biallelic_SNV/ALL.chr${i}.shapeit2_integrated_v1a.GRCh38.20181129.phased.vcf.gz.tbi"
done
```
...
vcf_files=$(ls data/vcf/chr*.vcf.gz | sort -V)
...
bcftools concat \
  ...
  --output-file data/vcf/1000G_subset.vcf.gz \
  --output-type z \
  --threads $threads
...
sample_info |> 
  ...
  mutate(chronotype = ifelse(chronotype == 0, 1, 2)) |> 
...
```bash
bcftools view -h data/vcf/1000G_subset.vcf.gz > temp.vcf
bcftools view -H data/vcf/1000G_subset.vcf.gz | awk '
BEGIN {srand(20250301); OFS="\t"} 
{
    # 20% of SNPs get a missing rate up to 30%, others get 0%
    missing_rate = (rand() < 0.2) ? rand() * 0.3 : 0  
    
    for (i=10; i<=NF; i++) {  
        if (rand() < missing_rate) {  
            sub(/[0-9]\/[0-9]|[0-9]\|[0-9]/, "./.", $i)
        }
    }
    print
}' >> temp.vcf
...
```bash
# temporary FAM file setting genotypes to missing
