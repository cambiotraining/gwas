...
Populations from different geographic regions may genetically diverge from each other due to evolutionary processes such as drift, selection, bottlenecks, etc. 
...
eigenvec |> 
  ...
  labs(x = paste0("PC1 (", round(eigenval$pct_var[1]), "%)"), 
       y = paste0("PC1 (", round(eigenval$pct_var[1]), "%)"), 
       colour = "World Region")
```
...
This population stratification will have to be taken into account when we run our association analysis, which is the topic of the next chapter. 
